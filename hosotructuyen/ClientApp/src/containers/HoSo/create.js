...
import { notify, listLoaiDinhKem } from '../../utils';
...
function NopHoSo({ token, userInfo }) {
  ...
  async function uploadFileWithInfo(hoSoID, category, _fileList, successFunc = function () {}) {
    if (_fileList.length > 0) {
      ...
      await Axios.post(urlFile + '/hoso/' + hoSoID + '/' + category + '/' + userInfo.UserId, fd, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: 'Bearer ' + token,
        },
      ...
      await _fileList.forEach((currentFile) => {
        ...
        works.push(
          uploadFileWithFormData(
            `${urlFile}/hoso/${hoSoID}/${currentFile.Category}/${userInfo.UserId}/single`,
            fd,
            successFunc,
          ),
        );
    ...
    return new Promise((resolve) => {
      hoSoForm
        ...
        .then(() => {
          ...
          console.debug('Dữ liệu file trình nộp (flat)', filesArray);
        })
        .then(() => {
  ...
  async function handleUploadFileTrinhNop(hoSoData) {
    ...
    let result = await uploadFileWithInfoOneByOne(idCreate, filesArray, function () {
  ...
  async function handleNopHoSo(idCreate) {
    await hoSoForm
      ...
      .then((hoSoData) => {
        ...
        //}
        //fd.append("filesInfo", JSON.stringify(filesInfo));
        let preWorks = [handleUploadFile(hoSoData)];
  ...
  function UploadProgress() {
    return (
      <>
        ...
        <div>
          <Text
            type={uploadedFilesCount > 0 && uploadedFilesCount === totalFilesCount ? 'success' : ''}
          >
            Đã tải lên {uploadedFilesCount}/{totalFilesCount} file đính kèm (
            {Math.round((uploadedFilesCount / totalFilesCount) * 100)}%)
          </Text>
        </div>
      </>
    );
      ...
      },
      {
        ...
        render: (value, rowData) => (
          <>
            ...
            <Row>
              <Button size="small" type="dashed" onClick={() => handleModalGoiY(rowData, true)}>
                Gợi ý
              </Button>
            </Row>
          </>
  ...
  function TaiLieuStaticFormList({ loaiTaiLieu }) {
    ...
    const autoCompleteData = listGoiYHoSo
        ...
        {(fields, { add, remove }) => (
          <>
            ...
            <Form.Item style={{ marginTop: 4, marginBottom: 4 }}>
              <Button type="lightdark" onClick={() => add()} block icon={<PlusCircleOutlined />}>
                Thêm tài liệu
              </Button>
            </Form.Item>
          </>
  ...
  return (
    <Col>
      <Modal
        title="Chưa điền đủ hồ sơ gợi ý"
        visible={isModalOpen}
        onOk={() => {
          handleNopHoSo(idCreate);
        }}
        ...
        cancelText="Thoát"
      >
        <p>Bạn chưa điền đầy đủ hồ sơ gợi ý nộp. Bạn có chắc chắn vẫn nộp hồ sơ này?</p>
      </Modal>
      ...
      >
        <Row gutter={[40, 40]}>
          <Col span={12}>
            ...
            <Form.Item
              name="NghienCuuVien"
              label="Tên nghiên cứu viên chính / Chủ nhiệm đề tài"
              rules={[{ required: true, message: 'Thông tin bắt buộc' }]}
              hasFeedback
            >
              <Input />
            ...
            <Form.Item
              name="CoQuanThucHien"
              label="Cơ quan thực hiện / Chủ trì đề tài"
              rules={[{ required: true, message: 'Thông tin bắt buộc' }]}
              hasFeedback
            >
              <Input />
            ...
            <Form.Item
              ...
              label={
                <>
                  Mô tả chi tiết{' '}
                  <Text type="secondary" style={{ fontWeight: '500', fontSize: 12, marginLeft: 2 }}>
                    (Không bắt buộc)
                  </Text>
                </>
              }
          ...
          <Col span={12}>
            ...
            <Row>
              <Col span={showLanBoSung === true ? 12 : 24}>
                <Form.Item
                  ...
                  hasFeedback
                >
                  <Select disabled onChange={handleLoaiHoSo} placeholder="Chọn phân loại hồ sơ">
                    <Option value="Nộp ban đầu">Nộp ban đầu</Option>
                    <Option value="Nộp bổ sung">Nộp bổ sung</Option>
                    <Option value="Nộp nghiệm thu">Nộp nghiệm thu</Option>
                  </Select>
                </Form.Item>
              </Col>
              <Col span={showLanBoSung === true ? 1 : 0}></Col>
              <Col span={showLanBoSung === true ? 11 : 0}>
                <Form.Item name="LanBoSung" label="Lần bổ sung" rules={lanBoSungRules} hasFeedback>
                  <InputNumber />
                </Form.Item>
              </Col>
            </Row>
            <Form.Item
              ...
              hasFeedback
            >
              <Select placeholder="Chọn phân loại đề tài" onChange={handleLoaiDeTai}>
            ...
            <Form.Item name="GiaiDoanThuNghiem" label="Đề nghị được thử nghiệm lâm sàng giai đoạn">
              <Select placeholder="Chọn giai đoạn" allowClear={true}>
            ...
            <Form.Item
              name="NgayNopHoSoGiay"
              label="Ngày hộp hồ sơ giấy tới hội đồng"
              rules={[{ required: true, message: 'Thông tin bắt buộc' }]}
              hasFeedback
            >
              <DatePicker
            ...
            <Form.Item
              name="NgayHdddChapThuan"
              label={
                <>
                  Ngày được hội đồng chấp thuận lần đầu{' '}
                  <Text type="secondary" style={{ fontWeight: '500', fontSize: 12, marginLeft: 2 }}>
            ...
            <Row>
              <Col span={8} style={{ marginRight: 10 }}>
                <Form.Item
                  ...
                  label={
                    <>
                      Lệ phí &nbsp; <ButtonGoiYLePhi />
                    </>
                  }
                >
                  <Select allowClear placeholder="Chọn mức">
        ...
        <Row gutter={[40, 0]} style={{ marginBottom: 20 }}>
          <Col span={24}>
            <Text style={{ fontSize: 15, fontWeight: 'bold' }}>Hồ sơ trình nộp</Text>
          </Col>
          <Col span={24}>
        ...
        <Row style={{ marginTop: 20 }} gutter={[40, 0]}>
          <Col span={24}>
            <Text style={{ fontSize: 15, fontWeight: 'bold' }}>Thông tin người nộp hồ sơ</Text>
          </Col>
          <Col span={12}>
            <Form.Item
              name="NguoiNopHoTen"
