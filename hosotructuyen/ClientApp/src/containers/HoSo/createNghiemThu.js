...
function NopHoSoBoSung({ token, userInfo }) {
  ...
  let { id } = useParams(); // id: Nộp bổ sung cho hồ sơ id này
    ...
    await Axios({
      ...
      .then((res) => {
        ...
        hoSoForm.setFieldsValue({
          ...
          //KinhPhiDuKien: hoSoGoc.KinhPhiDuKien,
          //GiaiDoanThuNghiem: "Giai đoạn 1",
          //NgayNopHoSoGiay: moment().endOf("month"),
          //NgayHdddChapThuan: hoSoGoc.NgayHdddChapThuan,
          //NgayTaoHoSo: moment().format("DD-MM-YYYY HH:mm"),
          LePhi: hoSoGoc.LePhi,
          NguoiNopHoTen: hoSoGoc.NguoiNopHoTen,
          NguoiNopEmail: hoSoGoc.NguoiNopEmail,
          NguoiNopPhone: hoSoGoc.NguoiNopPhone,
          NguoiNopDiaChi: hoSoGoc.NguoiNopDiaChi,
        });
      })
      .then(() => {
              ...
              if (uploadedFilesCount === totalFilesCount) {
                Axios({
                  ...
                  .then((result) => {
                    ...
                    if (result.data.Error !== true) {
                      handleCreateNotification(idCreate, tenDeTai);
                      //history.goBack();
                      setTimeout(() => {
                        setLoading(false);
                        setIsDone(true);
  ...
  if (isDone) {
    return (
      <Result
        status="success"
        title="Nộp hồ sơ bổ sung thành công"
...
export default connect(mapStateToProps, null)(NopHoSoBoSung);
